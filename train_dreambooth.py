...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="fp16",
        required=False,
        help="model revision from huggingface.co/models.",
    )
    parser.add_argument(
        "--tokenizer_name",
        type=str,
...
def main():
    ...
    if args.with_prior_preservation:
        ...
        if cur_class_images < args.num_class_images:
            ...
            pipeline = StableDiffusionPipeline.from_pretrained(
                args.pretrained_model_name_or_path, use_auth_token=args.use_auth_token, torch_dtype=torch_dtype, revision=args.revision
            )
    ...
    if args.tokenizer_name:
        ...
        tokenizer = CLIPTokenizer.from_pretrained(
            args.pretrained_model_name_or_path, subfolder="tokenizer", use_auth_token=args.use_auth_token, revision=args.revision
        )

    # Load models and create wrapper for stable diffusion
    text_encoder = CLIPTextModel.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="text_encoder", use_auth_token=args.use_auth_token, revision=args.revision
    )
    vae = AutoencoderKL.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="vae", use_auth_token=args.use_auth_token, revision=args.revision
    )
    unet = UNet2DConditionModel.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="unet", use_auth_token=args.use_auth_token, revision=args.revision
    )
